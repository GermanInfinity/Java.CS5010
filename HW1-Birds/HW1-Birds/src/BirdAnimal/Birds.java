...
/**
 * This abstract class represents an abstraction of methods and attributes for
 * different kinds of birds. The different kinds of birds include: Shorebirds,
 * Waterfowl, Parrots, Birds of Prey, Owls, Pigeons and flightless birds. This
 * class implements a superclass each of these kinds of birds can inherit from
 * and extend.
 * 
 */
...
public abstract class Birds {
  ...
  public String[] FoodPrefList = { "berries", "seeds", "fruit", "insects", "other birds", "eggs",
  ...
  /**
   * Concrete method: Set's the food preference of a bird object. Accepts an
   * array of strings and returns nothing.
   * 
   */
  ...
  /**
   * Concrete method: Adds food to food preference of a bird object. Accepts a
   * strings and returns nothing.
   * 
   */
  protected void addFoodPref(String PrefFood) {

    if (Arrays.asList(FoodPrefList).contains(PrefFood) == false) {
      throw new IllegalArgumentException(PrefFood + " is not a food choice for birds.");
    }
    ...
    if (add) {
      if (this.FoodPref.size() == 4) {
        throw new IllegalArgumentException("A bird cannot have more than 4 food preferences.");
      }
  ...
  /**
   * Concrete method: Remove a food preference of a bird object. Accepts a
   * strings and returns nothing.
   * 
   */
  protected void removeFoodPref(String PrefFood) {
    ...
    if (Arrays.asList(FoodPrefList).contains(PrefFood) == false) {
      throw new IllegalArgumentException(PrefFood + " is not a Bird food.");
    }

    if (this.FoodPref.contains(PrefFood)) {
      if (this.FoodPref.size() == 2) {
        throw new IllegalArgumentException("A bird cannot have less than 2 food preferences.");
      }
