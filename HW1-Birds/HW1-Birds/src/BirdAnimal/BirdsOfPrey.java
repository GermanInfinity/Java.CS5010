...
public class BirdsOfPrey extends Birds {
  ...
  private String[] BirdsOfPrey_Types = {"Hawks", "Eagles", "Osprey"};
  private String[] Bird_CharacList = {"Sharp beaks", "Hooked beaks"};
  
  
  /**
   * Constructs a BirdOfPrey bird object that has the type of bird, 
  ...
  protected void setType(String birdType) 
  {
    ...
    if (Arrays.asList(BirdsOfPrey_Types).contains(birdType) == false) {
  ...
  protected void removeCharac(String charac) 
  {
    ...
    {
      throw new IllegalArgumentException(
          charac + " is not a BirdsOfPrey Bird object characteristic.");
    }
                        ...
                        Boolean extinct, int numWings, ArrayList<String> prefFood) 
  {
     ...
     * Bird Type assertion. 
     */
    if (!Arrays.asList(BirdsOfPrey_Types).contains(birdType)) {
      throw new IllegalArgumentException(
          birdType + " is not a Flightless Bird.");
    }
