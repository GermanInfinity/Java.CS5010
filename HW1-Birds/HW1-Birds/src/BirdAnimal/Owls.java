...
public class Owls extends Birds {
  ...
  /**
   * Constructs a Owls bird object that has the type of bird, characteristics,
   * if it is existing, number of wings, and food preference.
   * 
   ...
   * 
   */
  public Owls(String birdType, ArrayList<String> characList, Boolean extinct, int numWings,
  ...
  protected void setType(String birdType) {
    ...
    if (Arrays.asList(OwlTypes).contains(birdType) == false) {
      throw new IllegalArgumentException(birdType + " is not a Owls Bird.");
    }
    this.TypeOfBird = birdType;
  }
  ...
  protected void setCharac(ArrayList<String> charac) {
    ...
    for (int i = 0; i < charac.size(); i++) {
      if (Arrays.asList(Bird_CharacList).contains(charac.get(i)) == false) {
        throw new IllegalArgumentException(charac.get(i) + " is not a Owls Bird characteristic.");
      }
    }
  ...
  protected void addCharac(String charac) {
    ...
    if (Arrays.asList(Bird_CharacList).contains(charac) == false) {
      throw new IllegalArgumentException(charac + " is not a Owls Bird characteristic.");
    }
  ...
  protected void removeCharac(String charac) {
    ...
    if (this.BirdCharac.contains(charac)) {
      ...
      throw new IllegalArgumentException(charac + " is not the Owls Bird objects characteristic.");
    }
